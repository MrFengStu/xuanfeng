...
<script src="${ctx }/resources/js/jquery-1.7.2.min.js"
...
<script type="text/javascript">
	...
	*/
	function search() {
		var p = $("#product_searchParam").val();
		window.location.href="${ctx }/product/list?product_searchParam="+p;
	}
	/*
	...
	*/
	$(function(){
		...
		$("[name='pagen']").each(function(key,value){
			$(this).attr("href",$(this).attr("href")+"&product_searchParam="+$("#product_searchParam").val());
		});
	})
	/*
		方法：详情清除缓存，
...
<body class="">
		...
		<div class="container-fluid">
			<div class="row-fluid">
				...
				<div class="btn-toolbar">
					...
					<input id="product_searchParam" type="text" value="${product_searchParam }" style="margin-left:2em;margin-top:0.55em;" />
    <button style="margin-left:1em;" onclick="javascrpt:search()"> search</button>
				...
				<div class="well">
					<table class="table">
						<thead>
							<tr>
								<th>#</th>
								<c:forEach items="${product_title}" var="keyword">
									<th>${keyword}</th>
								</c:forEach>
						...
						<tbody>
							<c:forEach items="${product_page.list}" var="p"
								...
								<tr>
									...
									<td>${p.price }</td>
									<td><a
										href="${ctx }/product/edit?productId=${p.productId }"><i
											class="icon-pencil"></i></a> <a role="button" data-toggle="modal"
										onclick="del('${p.productId }')"> <!-- 我修改的这里的a 标签
								 它原本的代码为 
								 href="#modal"
				...
				<div class="pagination">
					<ul>
						<c:forEach begin="1" end="${product_page.totalPageNum }"
							var="pageNum">
