...
public abstract class BaseDao<T, PK extends Serializable> {
	 ...
	 * 
	 */
	public List<String> findTableTitle(Object obj) throws Exception {

		List<String> re = new ArrayList<String>(0);
		Field[] field = obj.getClass().getDeclaredFields(); // 获取实体类的所有属性，返回Field数组
		// 遍历所有属性
		 for (int j = 0; j < field.length; j++) {
			 if(field[j].getName()=="userInfo"){
			 }else{
				 String temp=field[j].getName().substring(0, 1).toUpperCase()+field[j].getName().substring(1);
				 re.add(temp);
			 }
		 }
